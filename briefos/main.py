...
def main():
    ...
    try:
        ...
        log.warning("Wikipedia fetch failed (will continue without it): %s", exc)
        wiki = {"title": company_name, "summary": "", "categories": [], "url": ""}

    # ── Steps 3-4: Process + Render (stubs — implemented in next steps) ───
    log.info("Fetcher layer complete in %.1fs", time.time() - t0)
    log.info("Wikipedia article : %s", wiki["title"])
